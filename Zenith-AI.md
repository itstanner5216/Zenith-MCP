...
This guide provides everything a developer needs to understand, maintain, and extend the **Secure Filesystem MCP Server**. It is built using the **Model Context Protocol (MCP)** to provide LLMs with safe, high-performance, and semantically aware access to local filesystems.
...
1.  **In-Memory Validation:** Edits are verified against an in-memory string of the file.
    ...
    *   *Range Mode:* Strictly requires `verifyStart` and `verifyEnd` strings. The server asserts the trimmed contents of `startLine` and `endLine` match exactly to catch external file drift.
    *   *Symbol Mode:* Uses Tree-sitter to find the exact bounds of the symbol to replace.
...
*   The LLM can retry by sending only the corrected `oldText`, completely omitting `newText`. The tool rehydrates the `newText` from the cache and attempts the edit again.
...
| **directory_tree**      | JSON tree of directories/files.    | `path`, `excludePatterns`, `showSymbols` (Appends Tree-sitter metadata). Caps at 500 entries.                             |
