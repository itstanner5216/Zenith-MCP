...
2. Resolve repo root: `pc.getWorkingRoot(validPath)` — never null (falls back to `~/.zenith-mcp/workspace` when no project is detected; see [project-context-detection-methods.md](../project-scoping/project-context-detection-methods.md) for the materialization policy)
