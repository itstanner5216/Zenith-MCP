...
//   #6  v0 -> v1 migration is transactional (version advances only after all DDL)
...
describe('review #5: schema_version single-row determinism', () => {
    ...
    it('initSymbolSchema yields exactly one row with id=1, version=1', () => {
        db = openMemoryDb();
        initSymbolSchema(db);
...
describe('review #5: old-shape schema_version upgrade', () => {
    ...
    it('migrates an old-shape (no id column) table to single-row, preserving version=1', () => {
        db = openMemoryDb();
        initSymbolSchema(db);              // fully migrated, new-shape, v1
        ...
        expect(schemaVersionColumns(db)).toContain('id');
        expect(schemaVersionRowCount(db)).toBe(1);
        expect(queryRaw(db, 'SELECT id, version FROM schema_version')).toEqual([{ id: 1, version: 1 }]);
        expect(getSchemaVersion(db)).toBe(1);
    });
    ...
    it('re-runs the v0->v1 ladder when an old-shape table reports version 0', () => {
        ...
        initSymbolSchema(db);              // ladder should run and advance to 1

        expect(schemaVersionColumns(db)).toContain('id');
        expect(schemaVersionRowCount(db)).toBe(1);
        expect(getSchemaVersion(db)).toBe(1);
        // v1 child tables are present after the ladder.
        const tables = queryRaw(db, "SELECT name FROM sqlite_master WHERE type='table'").map((r) => r.name);
        for (const t of ['symbol_structures', 'anchors', 'imports', 'injections', 'local_scopes', 'project_roots']) {
...
describe('review #6: transactional migration', () => {
    ...
    it('initSymbolSchema commits atomically: version=1 and all v1 columns/tables exist', () => {
        db = openMemoryDb();
        initSymbolSchema(db);
    ...
    it('version stays 1 across repeated inits (ladder does not re-advance or duplicate)', () => {
        db = openMemoryDb();
        initSymbolSchema(db);
        initSymbolSchema(db);
        initSymbolSchema(db);
