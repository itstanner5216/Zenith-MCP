...
describe('def-node selection — decorated functions', () => {
    ...
    it('Python decorated function inside a class still gets its own range', async () => {
        // bar's @classmethod decorator is on line 6; def bar is on line 7;
        // bar's body ends on line 8. The class MyClass spans lines 5-8.
        // bar should have endLine === 8 (its body end), NOT inherit
        // MyClass.endLine (also 8 here, but the assertion checks the
        // selector reached function_definition, not class_definition).
        const source = [
            '',
            '',
            '',
