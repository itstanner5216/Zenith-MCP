...
describe('findProjectBoundary — ladder, junk filter, generation cache', () => {
    ...
    it('rejects a junk home git root before allowed-dir clamping can launder it', async () => {
        const { findProjectBoundary } = await importBoundaries();
        const fakeHome = mkdirp('junk-home-root');
        const allowed = path.join(fakeHome, 'workspace');
        fs.mkdirSync(path.join(fakeHome, '.git'), { recursive: true });
        fs.mkdirSync(allowed, { recursive: true });

        const homedirSpy = vi.spyOn(os, 'homedir').mockReturnValue(fakeHome);
        try {
            expect(findProjectBoundary(path.join(allowed, 'notes.txt'), {
