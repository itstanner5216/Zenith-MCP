...
describe('the scalar cursor retrieves the bytes it names', () => {
    ...
    /**
     * Documents written by hand, as JSON source text rather than through
     * `JSON.stringify`.
     *
     * Every fixture above this point was generated by serialising a JS string,
     * and a serialiser emits a deliberately small subset of the escapes JSON
     * permits: never `\uXXXX` for a character that has a literal form, never
     * `\/`. Those two are exactly the forms where a source byte count and a
     * re-serialised character count disagree — six bytes to one, two bytes to
     * one — so the walk could be measured against the wrong boundary at every
     * budget and no generated fixture could show it. The assertion below is not
     * decoration: it fails if these interiors ever drift back into the subset a
     * serialiser can produce, which is the only way this test quietly stops
     * testing anything.
     */
    const SOURCE_FIXTURES: readonly {
        readonly name: string;
        readonly interior: string;
        /**
         * Which disagreement the fixture carries. `width`: the source spends
         * more bytes on a character than a serialiser would, so a boundary
         * measured in serialised characters lands short. `dropped`: the decoder
         * discards a character it cannot emit, so a boundary measured from what
         * survives skips bytes the reader never saw. Only `width` fixtures can
         * be checked against the serialiser, because `JSON.stringify` emits
         * `\uXXXX` for a lone surrogate at exactly the width the source used.
         */
        readonly carries: 'width' | 'dropped';
    }[] = [
        // Six source bytes per character, the widest disagreement JSON allows.
        { name: 'u-escaped ascii', interior: Array.from({ length: 120 }, (_, i) => `\\u00${(0x41 + (i % 26)).toString(16)}`).join(''), carries: 'width' },
        // Two source bytes per character, and a form JSON.stringify never emits.
        { name: 'escaped solidus', interior: '\\/'.repeat(200), carries: 'width' },
        // Twelve source bytes per astral character, cut anywhere inside a pair.
        { name: 'u-escaped surrogate pairs', interior: '\\ud83d\\ude00'.repeat(60), carries: 'width' },
        // Six source bytes to two UTF-8 bytes: wrong in both directions at once.
        { name: 'u-escaped latin-1', interior: '\\u00e9'.repeat(150), carries: 'width' },
        // Interleaved widths, so no single stride can be right for the run.
        { name: 'every escape kind interleaved', interior: Array.from({ length: 120 }, (_, i) => ['\\u0041', '\\/', '\\n', '\\\\', '\\"', 'q', '\u00e9', '\\u00e9', '\u{1f600}', '\\ud83d\\ude00'][i % 10]).join(''), carries: 'width' },
        // A high surrogate with no partner: the decoder drops the character, so
        // it must give back the six bytes it came from as well.
        { name: 'lone high surrogate', interior: '\\ud83dab'.repeat(50), carries: 'dropped' },
    ];

    it('reconstructs a scalar written with escapes a serialiser cannot emit', () => {
        for (const { name, interior, carries } of SOURCE_FIXTURES) {
            const doc = `{"a":"${interior}"}`;
            const truth: unknown = (JSON.parse(doc) as Record<string, unknown>)['a'];
            expect(typeof truth, `${name}: the fixture is not valid JSON source`).toBe('string');
            if (typeof truth !== 'string') continue;
            if (carries === 'width') {
                expect(
                    interior === JSON.stringify(truth).slice(1, -1),
                    `${name}: the fixture's source text is byte-identical to what JSON.stringify would produce, so `
                    + 'it cannot reach the path where source bytes and serialised characters disagree. Write the '
                    + 'escapes by hand or this test is only re-checking the generated fixtures above.',
                ).toBe(false);
            }
            const resolver = open(doc, `scalar-source-${name}`);
            for (const budget of BUDGETS) {
                const walked = walkScalar(resolver, '/a', budget);
                expect(
                    walked.stalled,
                    `${name} at budget ${budget}: the walk stopped advancing after ${walked.steps} steps`,
                ).toBe(false);
                expect(
                    truth.startsWith(walked.text),
                    `${name} at budget ${budget}: the walk produced ${walked.text.length} characters that are not a `
                    + `leading run of the ${truth.length}-character value. Content that is not in the document at the `
                    + `position claimed is fabrication (I1). First divergence at `
                    + `${String([...walked.text].findIndex((c, i) => truth[i] !== c))}.`,
                ).toBe(true);
                expect(
                    walked.text,
                    `${name} at budget ${budget}: following the cursors recovered ${walked.text.length} of `
                    + `${truth.length} characters in ${walked.steps} steps. A boundary measured in serialised `
                    + 'characters instead of source bytes resumes inside an escape, and the escape\'s tail then '
                    + 'decodes as content.',
                ).toBe(truth);
            }
        }
    ...
    it('offers no continuation at a budget where none can advance, and says so', () => {
        // Swept rather than sampled: below some budget no offset can show a
        // byte, and that floor is not a constant. The marker carries a cursor,
        // the cursor encodes the pointer, so a deeper node has a wider marker
        // and a higher floor — measured at 96 for the pointer `/a` and 484 for
        // a 194-character pointer, which is well above MIN_BUDGET_CHARS. A
        // fixed list of budgets steps straight over the band; the sweep crosses
        // it, and the deep case below crosses the widest one measured, so a
        // regression shows up as a band rather than as one stray case.
        const shells: readonly { readonly label: string; readonly wrap: (v: string) => string; readonly pointer: string; readonly upTo: number }[] = [
            { label: 'shallow', wrap: (v) => `{"a":"${v}"}`, pointer: '/a', upTo: 320 },
            {
                label: 'deep pointer',
                wrap: (v) => `{"k0000":{"k0001":{"k0002":{"k0003":{"k0004":{"k0005":{"k0006":{"k0007":{"a":"${v}"}}}}}}}}`,
                pointer: '/k0000/k0001/k0002/k0003/k0004/k0005/k0006/k0007/a',
                upTo: 560,
            },
        ];
        for (const shell of shells) for (const { name, interior } of SOURCE_FIXTURES) {
            const resolver = open(shell.wrap(interior), `scalar-floor-${shell.label}-${name}`);
            const node = resolver.resolve(shell.pointer);
            for (let budget = MIN_VIEW_BUDGET; budget <= shell.upTo; budget++) {
                let offset = 0;
                for (let step = 0; step < 4_000; step++) {
                    const view = renderNode(resolver, node, { budget, offset });
                    assertUniversal(view, resolver.source.size, `${name} (${shell.label}) floor sweep at budget ${budget}`);
                    const next = view.envelope.next;
                    if (next === undefined) {
                        // Terminating with bytes still withheld is honest only
                        // if the view names the budget as the reason, because a
                        // caller cannot tell "that is all of it" from "none of
                        // it fit" any other way.
                        if (view.envelope.omitted.some((o) => o.of === 'bytes')) {
                            expect(
                                view.envelope.hint,
                                `${name} (${shell.label}) at budget ${budget}, offset ${offset}: the view withholds bytes, offers no `
                                + 'continuation, and its hint does not ask for a larger budget. That is a walk that '
                                + 'ends with no way forward and no way to know why.',
                            ).toContain('Raise the budget');
                        }
                        break;
                    }
                    const to = next.offset ?? 0;
                    expect(
                        to,
                        `${name} (${shell.label}) at budget ${budget}: the continuation from byte offset ${offset} addresses `
                        + `${to}. An address at or behind the one that produced it is a cycle, and the caller has no `
                        + 'way to detect it.',
                    ).toBeGreaterThan(offset);
                    expect(to, `${name} (${shell.label}) at budget ${budget}: continuation offset ${to} is not the one its cursor carries`)
                        .toBe(decodeCursor(next.cursor).offset);
                    offset = to;
                    expect(step, `${name} (${shell.label}) at budget ${budget}: the walk did not terminate`).toBeLessThan(3_999);
                }
            }
        }
    }, 240_000);
