 ...
 *   - src/utils/project-scope.ts (resolveProjectRoot, isWithinProject, clampToAllowed)
 *
...
// ═══════════════════════════════════════════════════════════════════════════════
// project-scope.ts — resolveProjectRoot
// ═══════════════════════════════════════════════════════════════════════════════
describe('resolveProjectRoot — resolution ladder', () => {
    // Branch: git root returned for file inside git repo
    it('returns git root for file in git repo', async () => {
        const { resolveProjectRoot, clearProjectScopeCache } = await import('../dist/utils/project-scope.js');
        clearProjectScopeCache();
        const repoDir = mkTmpGitRepo();
        const f = path.join(repoDir, 'index.js');
        fs.writeFileSync(f, 'module.exports = {}');
        try {
            const root = resolveProjectRoot(f, { allowedDirectories: [repoDir], noCache: true });
    ...
    // Branch: monorepo — deeper marker wins over git root
    it('prefers deeper marker in monorepo', async () => {
        const { resolveProjectRoot, clearProjectScopeCache } = await import('../dist/utils/project-scope.js');
        clearProjectScopeCache();
        const repoDir = mkTmpGitRepo();
        const pkgDir = path.join(repoDir, 'packages', 'lib');
        fs.mkdirSync(pkgDir, { recursive: true });
        fs.writeFileSync(path.join(pkgDir, 'package.json'), '{}');
        const f = path.join(pkgDir, 'src', 'index.ts');
    ...
    // Branch: no git, marker only
    it('returns marker root when no git repo exists', async () => {
        const { resolveProjectRoot, clearProjectScopeCache } = await import('../dist/utils/project-scope.js');
        clearProjectScopeCache();
        const tmpDir = mkTmpDir();
        fs.writeFileSync(path.join(tmpDir, 'package.json'), '{}');
        const f = path.join(tmpDir, 'src', 'main.js');
        fs.mkdirSync(path.join(tmpDir, 'src'));
        fs.writeFileSync(f, '');
        try {
            const root = resolveProjectRoot(f, { allowedDirectories: [tmpDir], noCache: true });
    ...
    // Branch: file outside all allowed directories → null
    it('returns null for file outside allowed directories', async () => {
        const { resolveProjectRoot, clearProjectScopeCache } = await import('../dist/utils/project-scope.js');
        clearProjectScopeCache();
        const allowed = mkTmpDir();
        const outside = mkTmpDir();
        const f = path.join(outside, 'file.js');
        fs.writeFileSync(f, '');
        try {
            const root = resolveProjectRoot(f, { allowedDirectories: [allowed], noCache: true });
    ...
    // Branch: no options → still resolves via git/markers
    it('resolves with no options at all', async () => {
        const { resolveProjectRoot, clearProjectScopeCache } = await import('../dist/utils/project-scope.js');
        clearProjectScopeCache();
        const repoDir = mkTmpGitRepo();
        const f = path.join(repoDir, 'file.py');
        fs.writeFileSync(f, '');
        try {
            const root = resolveProjectRoot(f, { noCache: true });
            expect(root).toBe(repoDir);
        } finally {
            fs.rmSync(repoDir, { recursive: true, force: true });
        }
    ...
    it('returns cached result on second call', async () => {
        ...
        clearProjectScopeCache();
        const repoDir = mkTmpGitRepo();
        const f = path.join(repoDir, 'x.js');
        fs.writeFileSync(f, '');
        try {
            const first = resolveProjectRoot(f, { allowedDirectories: [repoDir] });
            const second = resolveProjectRoot(f, { allowedDirectories: [repoDir] });
...
// ═══════════════════════════════════════════════════════════════════════════════
// project-scope.ts — isWithinProject
// ═══════════════════════════════════════════════════════════════════════════════
describe('isWithinProject — boundary cases', () => {
    it('returns true when path equals root', async () => {
        const { isWithinProject } = await import('../dist/utils/project-scope.js');
        expect(isWithinProject('/a/b', '/a/b')).toBe(true);
    });

    it('returns true for subdirectory', async () => {
        const { isWithinProject } = await import('../dist/utils/project-scope.js');
        expect(isWithinProject('/a/b/c', '/a/b')).toBe(true);
    });

    it('returns false for prefix collision', async () => {
        const { isWithinProject } = await import('../dist/utils/project-scope.js');
        expect(isWithinProject('/a/bcd', '/a/b')).toBe(false);
    });

    it('returns false for completely separate paths', async () => {
        const { isWithinProject } = await import('../dist/utils/project-scope.js');
        expect(isWithinProject('/x/y', '/a/b')).toBe(false);
    });

    it('handles relative paths by resolving', async () => {
        const { isWithinProject } = await import('../dist/utils/project-scope.js');
        const cwd = process.cwd();
        expect(isWithinProject('sub/file.txt', cwd)).toBe(true);
    });
});
