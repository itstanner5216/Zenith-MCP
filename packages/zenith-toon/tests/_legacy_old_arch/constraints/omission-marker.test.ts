  ...
  it('no internal omission gap is smaller than 6 lines', () => {
    for (const { name, source, out } of allStructuredOutputs()) {
      ...
      for (const m of markers) {
        ...
        // Every emitted marker stands for a >= 6-line omission: a smaller gap must
        // be shown, not marked (constraints.md §1 / Phase H 6-line floor).
        expect(span, `${name}: gap ${m.x}-${m.y} < 6`).toBeGreaterThanOrEqual(6);
      }
    }
  });
